...
|-----------|---------------|-------------|
| Task Parser | `src/tasks/taskParser.ts` | `extractTaskDescription()`, `parseLine()` |
| Cache Manager | `src/taskManager.ts` | `getAllTasks()`, `updateFileCache()` |
| Base Renderer | `src/calendar/BaseCalendarRenderer.ts` | `cleanTaskDescription()`, `renderTaskDescriptionWithLinks()` |
