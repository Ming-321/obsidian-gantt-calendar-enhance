...
**创建位置 (BaseCalendarRenderer.ts:430-447):**
```typescript
...
**优先级图标映射 (BaseCalendarRenderer.ts:48-57):**
```typescript
...
**DOM结构 (BaseCalendarRenderer.ts:176-246):**
```html
...
| Tooltip命名误导 | `calendar-week-task-tooltip` | 改为 `.task-tooltip` | BaseCalendarRenderer.ts |
...
```typescript
// 修改文件: BaseCalendarRenderer.ts, WeekView.ts, MonthView.ts
...
2. 更新 `BaseCalendarRenderer.ts` 中的 `renderTaskDescriptionWithLinks()` 调用
3. 更新各视图的文本容器创建代码
4. 测试所有视图
5. 移除旧类名样式
...
```typescript
// 修改文件: BaseCalendarRenderer.ts:176
