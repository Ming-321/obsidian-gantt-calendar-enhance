...
- Bar 颜色按优先级区分（v2.1: 统一优先级色，不再按类型区分颜色）：赤陶橙=高、清澈蓝=普通、灰色=低、翠绿=已完成；提醒类型通过斜体标题区分
- 交互（v2.1 更新）：hover 显示 tooltip、有子条目的标题显示下划线、左键单击展开/折叠子条目、右键打开编辑菜单、支持状态/标签筛选、待办任务显示可点击 StatusIcon
- 层级区分（v2.1 新增）：子任务 bar 文字缩进20px、孙任务文字缩进34px；子/孙任务条尺寸略小于父任务条
...
- 不显示任务类型标签（待办/提醒）
- 背景透明，左对齐（v2.1 更新：从白色居中改为透明左对齐）
- 交互（v2.1 更新）：有子条目的标题显示下划线、左键单击展开/折叠、右键打开编辑菜单
